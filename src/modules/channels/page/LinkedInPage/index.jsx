...
import { LinkedInPagePerformanceList } from '../../../../constant/app/channel';
...
import { linkedinPageGraph } from '../../api/Api';
...
const LinkedInPage = () => {
  ...
  const [overviewList, setOverviewList] = useState(LinkedInPagePerformanceList);
  ...
  useEffect(() => {
    const selectedTab = channelList?.filter((list) => list?.label === 'linkedin');

    if (selectedTab?.length > 0 && selectedTab[0]?.apiData?.status === 1) {
      linkedinPageGraph(currentBrand?.id).then((res) => assignData(res));
    }
  }, [currentBrand, channelList]);

  const assignData = (res) => {
    if (res?.status === 200) {
      const dividedArray = divideArray(res?.data?.list, 4);
      let retrievedData = [];
      dividedArray?.map((list) => {
        retrievedData.unshift({
          name: moment(list[0]?.end_date).format('MMM D'),
          value: list[0]?.total_size,
        });
      });
      setGraphData(retrievedData);
    }
    const overViewList = LinkedInPagePerformanceList?.map((item) => {
      if (item?.title === 'Follower') {
        return { ...item, content: Math.ceil(res?.data?.followers) };
      } else if (item?.title === 'Mentions') {
      ...
      } else if (item?.title === 'Page views') {
        return { ...item, content: Math.ceil(res?.data?.stats?.engagement) };
      } else if (item?.title === 'Unique impressions') {
